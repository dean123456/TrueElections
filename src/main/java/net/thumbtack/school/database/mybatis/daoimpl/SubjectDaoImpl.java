...
public class SubjectDaoImpl extends DaoImplBase implements SubjectDao {
    ...
    public Subject insert(Subject subject) {
        LOGGER.debug("Add subject {} to Database", subject);
        try (SqlSession sqlSession = getSession()) {
            try {
                getSubjectMapper(sqlSession).insert(subject);
            } catch (RuntimeException ex) {
                LOGGER.info("Can't add subject {} to Database. {}", subject, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
    ...
    public Subject update(Subject subject) {
        LOGGER.debug("Update subject to {} ", subject);
        try (SqlSession sqlSession = getSession()) {
            try {
    ...
    public void delete(Subject subject) {
        LOGGER.debug("Delete subject {}", subject);
        try (SqlSession sqlSession = getSession()) {
            try {
    ...
    public void deleteAll() {
        LOGGER.debug("Delete all subjects");
        try (SqlSession sqlSession = getSession()) {
            try {
